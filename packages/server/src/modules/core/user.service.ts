...
import { LoginDto, LocalStrategyInfo, LoginResponse, ProfileResponse } from './dto/login.dto';
...
export class UserService {
    ...
    async login(loginDto: LoginDto): Promise<LoginResponse | false> {
        ...
        const result: LoginResponse | any | false = await this.validate(request, response, next);
    ...
    private async validate(request: Request, response: Response, next: NextFunction): Promise<LoginResponse | any | false> {
        return await new Promise((resolve, reject) => {
            passport.authenticate('local', (err: Error, user: LoginResponse, info: LocalStrategyInfo) => {
                if (err) {
                    reject(err);
                }
                if (user) {
