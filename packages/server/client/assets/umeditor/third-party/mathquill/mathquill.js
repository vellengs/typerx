 ...
 */
(function () {
    var jQuery = window.jQuery, undefined, _, //temp variable of prototypes
    mqCmdId = 'mathquill-command-id', mqBlockId = 'mathquill-block-id', min = Math.min, max = Math.max;
    var __slice = [].slice;
    function noop() { }
    /**
    ...
    var MathElement = P(Node, function (_, _super) {
        ...
        this.jQize = function (html) {
            ...
            jQ.find('*').andSelf().each(function () {
                var jQ = $(this), cmdId = jQ.attr('mathquill-command-id'), blockId = jQ.attr('mathquill-block-id');
                if (cmdId)
                    MathElement[cmdId].jQadd(jQ);
                if (blockId)
                    MathElement[blockId].jQadd(jQ);
            });
            return jQ;
        };
    ...
    function createRoot(jQ, root, textbox, editable) {
        ...
        //textarea stuff
        var textareaSpan = root.textarea = $('<span class="textarea"><textarea></textarea></span>'), textarea = textareaSpan.children();
        /******
    ...
    var SupSub = P(MathCommand, function (_, _super) {
        ...
        _.respace = function () {
            if (this[L].ctrlSeq === '\\int ' || (this[L] instanceof SupSub && this[L].ctrlSeq != this.ctrlSeq
            ...
            if (this.respaced) {
                var fontSize = +this.jQ.css('fontSize').slice(0, -2), leftWidth = this[L].jQ.outerWidth(), thisWidth = this.jQ.outerWidth();
                this.jQ.css({
                ...
                if (!this.replacedFragment) {
                    ...
                    while (leftward &&
                        !(leftward instanceof BinaryOperator ||
                            ...
                            ',;:'.split('').indexOf(leftward.ctrlSeq) > -1) //lookbehind for operator
                    )
                        leftward = leftward[L];
    ...
    var Bracket = P(MathCommand, function (_, _super) {
        _.init = function (open, close, ctrlSeq, end) {
            _super.init.call(this, '\\left' + ctrlSeq, '<span class="non-leaf">'
    ...
    var InnerTextBlock = P(MathBlock, function (_, _super) {
        ...
        _.focus = function () {
            ...
            if (textblock[R].ctrlSeq === textblock.ctrlSeq) {
                var innerblock = this, cursor = textblock.cursor, rightward = textblock[R].ends[L];
    ...
    var LatexCommandInput = CharCmds['\\'] = P(MathCommand, function (_, _super) {
        ...
        _.createLeftOf = function (cursor) {
            ...
            if (this._replacedFragment) {
                var el = this.jQ[0];
                this.jQ =
                    this._replacedFragment.jQ.addClass('blur').bind('mousedown mousemove', //FIXME: is monkey-patching the mousedown and mousemove handlers the right way to do this?
                    function (e) {
    ...
    */
    ...
    var Cursor = P(Point, function (_) {
        ...
        _.moveDirWithin = function (dir, block) {
            prayDirection(dir);
            if (this[dir]) {
                if (this[dir].ends[-dir])
                    this.insAtDirEnd(-dir, this[dir].ends[-dir]);
                else
                    this.hopDir(dir);
            }
            else {
        ...
        function moveUpDown(self, dir) {
            if (self[R][dir])
                self.insAtLeftEnd(self[R][dir]);
            else if (self[L][dir])
                self.insAtRightEnd(self[L][dir]);
            else {
        ...
        _.selectDir = function (dir) {
            ...
            if (this.selection) {
                ...
                if (this.selection.ends[dir] === this[-dir]) {
                    ...
                    if (this[dir])
                        this.hopDir(dir).selection.extendDir(dir);
                    else if (this.parent !== this.root) {
    ...
    var Selection = P(MathFragment, function (_, _super) {
        ...
        _.levelUp = function () {
            var seln = this, gramp = seln.ends[L] = seln.ends[R] = seln.ends[R].parent.parent;
