...
import { XlsxService, SimpleTableColumn, STExportOptions, SimpleTableData, ReqReNameType, ResReNameType, SimpleTableMultiSort, SimpleTableChange, AdSimpleTableConfig, SimpleTableButton, SimpleTableSelection, SimpleTableFilter, CompareFn } from '@delon/abc';
...
export class BaseTableComponent extends BaseComponent implements BaseTable, OnInit, OnDestroy, OnChanges {

    domain: string;
    columnSets: { [key: string]: SimpleTableColumn[]; };
    queryParams: { [key: string]: any; };

    private data$: Subscription;
    private _inited = false;

    tableDataSource: SimpleTableData[] = [];
    _url: string;
    _isAjax = false;
    _isPagination = true;
    _classMap: string[] = [];
    ...
    /** 页码、每页数量变化时回调 */
    @Output()
    change: EventEmitter<SimpleTableChange> = new EventEmitter<SimpleTableChange>();
    /** checkbox变化时回调，参数为当前所选清单 */
    @Output()
    checkboxChange: EventEmitter<SimpleTableData[]> = new EventEmitter<SimpleTableData[]>();
    /** radio变化时回调，参数为当前所选 */
    @Output()
    radioChange: EventEmitter<SimpleTableData> = new EventEmitter<SimpleTableData>();
    /** 排序回调 */
    @Output() sortChange: EventEmitter<any> = new EventEmitter<any>();
    /** Filter回调 */
    @Output()
    filterChange: EventEmitter<SimpleTableColumn> = new EventEmitter<SimpleTableColumn>();
    ...
    _get(item: any, col: SimpleTableColumn) {
        ...
        switch (col.type) {
            ...
            case 'number':
                ...
                break;
            case 'currency':
                ret = this.currency.transform(value);
                break;
            case 'date':
    ...
    _checkAll(checked?: boolean): this {
        ...
        this.tableDataSource.filter(w => !w.disabled).forEach(i => (i.checked = checked));
    ...
    _refCheck(): this {
        const validData = this.tableDataSource.filter(w => !w.disabled);
    ...
    _rowSelection(row: SimpleTableSelection): this {
        row.select(this.tableDataSource);
        return this._refCheck()._checkNotify();
    }

    _checkNotify(): this {
        this.checkboxChange.emit(
            this.tableDataSource.filter(w => !w.disabled && w.checked === true),
        );
        return this;
    }
    ...
    /** 清除所有 `radio` */
    clearRadio(): this {
        this.tableDataSource.filter(w => w.checked).forEach(item => (item.checked = false));
    ...
    _refRadio(idx: number, checked: boolean, item: SimpleTableData): this {
        ...
        this.tableDataSource.filter(w => !w.disabled).forEach(i => (i.checked = false));
        item.checked = checked;
    ...
    btnClick(record: any, btn: SimpleTableButton) {
        if (btn.type === 'modal' || btn.type === 'static') {
            const obj = {};
            obj[btn.paramName || 'record'] = record;

            (this.modalHelper[btn.type === 'modal' ? 'open' : 'static'] as any)(
                btn.component,
    ...
    private updateColumns() {
        ...
        const copyColumns = deepCopy(this.columns) as SimpleTableColumn[];
